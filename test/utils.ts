...
export const createMockPlexResponse = (songs: Partial<Song>[] = []) => ({
  MediaContainer: {
    Metadata: songs.map((song) => ({
      ...
      thumb:
        song.thumb?.replace("/api/images/", "") ||
        "/library/metadata/12345/thumb/1234567890",
      ...
      parentThumb:
        song.parentThumb?.replace("/api/images/", "") ||
        "/library/metadata/67890/thumb/1234567890",
...
export const validateImageUrlRewrite = (
  ...
  const cleanPath = originalUrl.startsWith("/")
    ? originalUrl.slice(1)
    : originalUrl;
...
export const buildPlexUrl = (searchParams: string = "") => {
  const baseUrl = `${process.env.VITE_PLEX_SERVER_URL}/library/sections/3/all`;
